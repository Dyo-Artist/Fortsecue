...
def test_update_merge_thresholds_clamps_to_unit_interval(tmp_path):
    base = tmp_path / "kb"
    store = KnowledgebaseStore(base_path=base, actor="tester")
...
""",
    ...
    applied = store.update_merge_thresholds({"name_similarity": 0.05, "org_similarity": -0.1}, scope="defaults")

    assert applied["name_similarity"] == 1.0
    assert applied["org_similarity"] == 0.0

    data = yaml.safe_load(rules_path.read_text())
    assert data["defaults"]["name_similarity"] == 1.0
    assert data["defaults"]["org_similarity"] == 0.0
